      ...
      {selectedImage ? (
        ...
        <div
          style={{
            display: "flex",
            flexWrap: "wrap",
            backgroundImage: "radial-gradient(rgb(60,60,60), rgb(20,20,20))"
          }}
        >
          {images.map(({ download_url, author, height, width }, index) => {
            const size = getCorrectSize(width, height);

            return (
              <ImageCell
                onClick={() => setSelectedImage(download_url)}
                key={index}
              >
                <Image
                  animate
...
const ImageCell = styled(motion.div)`
  display: flex;
...
function getCorrectSize(width: number, height: number) {
  if (width <= 200 && height <= 200) {
    return { width, height };
  }
  const ratio = height / 120;
  return { width: width / ratio, height: height / ratio };
}

function getRandomImages(): ImageData[] {
